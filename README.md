...
11. If `Stop` was invoked in step 7 or in step 8; or if the instruction was RET and `AutoStopOnRetWithStackEmpty` is true; or if the instruction was HALT, interrupts are disabled and `ÀutoStopOnDiPlusHalt` is true, the `Stop` method returns.
